import React from 'react';
import { useLanguage } from '../lib/i18n';
import { ABOUT_CONTENT } from '../data/aboutContent';

interface AboutPageProps {
  onGoHome: () => void;
}

export const AboutPage: React.FC<AboutPageProps> = ({ onGoHome }) => {
  const { language } = useLanguage();
  const content = ABOUT_CONTENT[language] || ABOUT_CONTENT.es;

  return (
    <div style={{
      maxWidth: '820px',
      margin: '0 auto',
      padding: '3rem 1.5rem 5rem 1.5rem',
      color: 'var(--color-navy)',
      fontFamily: 'var(--font-sans)',
      direction: language === 'ar' ? 'rtl' : 'ltr'
    }}>
      {/* Return Link */}
      <button
        onClick={onGoHome}
        style={{
          background: 'none',
          border: 'none',
          padding: 0,
          fontSize: '0.95rem',
          fontWeight: 600,
          color: '#0284C7',
          cursor: 'pointer',
          marginBottom: '2.5rem',
          display: 'inline-block'
        }}
      >
        {content.backToClock}
      </button>

      {/* Main Header */}
      <header style={{ marginBottom: '2.5rem', borderBottom: '1px solid var(--color-border)', paddingBottom: '2rem' }}>
        <h1 style={{
          fontSize: '2.5rem',
          fontWeight: 900,
          color: 'var(--color-navy)',
          margin: '0 0 0.75rem 0',
          letterSpacing: '-0.025em',
          lineHeight: 1.15
        }}>
          {content.title}
        </h1>
        <p style={{
          fontSize: '1.2rem',
          color: '#0284C7',
          fontWeight: 500,
          margin: 0,
          lineHeight: 1.6
        }}>
          {content.subtitle}
        </p>
      </header>

      {/* Article Body */}
      <article style={{ fontSize: '1.05rem', lineHeight: 1.85, color: '#334155' }}>
        <section style={{ marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-navy)', margin: '0 0 1rem 0', letterSpacing: '-0.01em' }}>
            {content.visionTitle}
          </h2>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.visionP1}
          </p>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.visionP2}
          </p>
        </section>

        <section style={{ marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-navy)', margin: '0 0 1rem 0', letterSpacing: '-0.01em' }}>
            {content.pillarsTitle}
          </h2>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.pillarsIntro}
          </p>
          <p style={{ marginBottom: '1rem' }}>
            <strong>{content.pillar1Title}</strong> {content.pillar1Desc}
          </p>
          <p style={{ marginBottom: '1rem' }}>
            <strong>{content.pillar2Title}</strong> {content.pillar2Desc}
          </p>
          <p style={{ marginBottom: '1rem' }}>
            <strong>{content.pillar3Title}</strong> {content.pillar3Desc}
          </p>
          <p style={{ marginBottom: '1.25rem' }}>
            <strong>{content.pillar4Title}</strong> {content.pillar4Desc}
          </p>
        </section>

        <section style={{ marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-navy)', margin: '0 0 1rem 0', letterSpacing: '-0.01em' }}>
            {content.techTitle}
          </h2>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.techP1}
          </p>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.techP2}
          </p>
        </section>

        <section style={{ marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-navy)', margin: '0 0 1rem 0', letterSpacing: '-0.01em' }}>
            {content.storyTitle}
          </h2>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.storyP1}
          </p>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.storyP2}
          </p>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.storyP3}
          </p>
        </section>

        <section style={{ marginBottom: '1rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-navy)', margin: '0 0 1rem 0', letterSpacing: '-0.01em' }}>
            {content.i18nTitle}
          </h2>
          <p style={{ marginBottom: '1.25rem' }}>
            {content.i18nP1}
          </p>
        </section>
      </article>
    </div>
  );
};
